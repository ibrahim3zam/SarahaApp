...
import { updateCoverImagesSchema, uploadImageSchema } from "../authModule/auth.validation.js";
...
router.get("/getUser", auth(), allowRoles(Roles.admin), userServices.getProfile)
...
router.patch("/cover-images", auth(), uploadFile({ type: fileType.Image }).array("images", 5), validation(updateCoverImagesSchema), userServices.updateCoverImages);
router.patch("/soft-delete/:id", auth(), userServices.softDeleteUser);
router.patch("/restore/:id", auth({ activation: false }), userServices.restoreUser);
router.delete("/hard-delete/:id", auth(), userServices.hardDeleteUser);
router.patch("/upload-profile", auth(), cloudUploadFile({ type: fileType.Image }).single("image"), validation(uploadImageSchema), userServices.uploadProfileImage);
