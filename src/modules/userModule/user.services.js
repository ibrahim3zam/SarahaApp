...
export const updateUser = async (req, res) => {
  ...
  const updatedUser = await userDB.updateOne({ _id: user._id }, updatedData, { new: true });
...
export const uploadProfileImage = async (req, res) => {
  ...
  const { secure_url, public_id } = await uploadSingleFile({ path: file.path, folder: `Users/${user.name}_${user._id}/Profile` });
...
export const updateCoverImages = async (req, res) => {
  ...
  for (const file of req.files) {
    const { secure_url, public_id } = await uploadSingleFile({ path: file.path, folder: `Users/${req.user.name}_${req.user._id}/Cover` });
